...
## Description
Lightweight MCP server providing workflow tools for Claude Code. 17 tools covering workflow management, verification, self-correction, adaptive replanning, and project memory.
...
- `src/claude_orchestrator/fixer.py` -- Self-correcting verification analyzer with convergence tracking
- `src/claude_orchestrator/replanner.py` -- Adaptive replanning (mutable plans during execution)
...
- `tests/test_e2e.py` -- End-to-end tests (121 tests)
- `eval/scenarios.py` -- Eval scenarios (46 across 13 dimensions)
- `eval/baseline.json` -- Eval baseline (100% pass rate)

## Constraints
- Tool count is currently 17 -- tests assert this; update test if adding/removing tools
